<!-- P2M_REPORT -->
<!-- GENERATED at 2025-11-11 22:43:54 -->
...
- Files: **36**
- Total size: **38454 bytes**
- Total LOC: 1157 | SLOC: 969 | TODOs: 0
...
- `frontend/app/screens/HomeScreen.tsx` — 2645 bytes
...
- `.gitignore` — 207 LOC
- `LICENSE` — 201 LOC
- `frontend/app/screens/HomeScreen.tsx` — 82 LOC
...
- Size: 2645 bytes | LOC: 82 | SLOC: 65 | TODOs: 0 | Modified: 2025-11-11 22:35:28 | SHA1: d060f9b3aef5
...
export default function HomeScreen() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const appConfig = await webllm.CreateMLCEngine(
        // 例: Llama3系など、公式app-configを利用
        "Llama-3.2-3B-Instruct-q4f16_1-MLC",
        {
          // GitHub Pages上に置く model/ 以下を参照させる
          model_list_url: "/client_side_llm/webllm/model-config.json",
        }
      );
      setEngine(appConfig);
      setReady(true);
  ...
  async function handleAsk() {
    ...
    const context = await retrieveContext(question); // ← 自前RAG（埋め込み検索）

    const messages: webllm.ChatCompletionMessageParam[] = [
      {
        role: "system",
        content:
          "You are a RAG assistant. Answer ONLY using the given context. If unclear, say you don't know.",
      },
      {
        role: "user",
        content:
  ...
  return (
    <View style={{ flex: 1, padding: 16 }}>
      <Text>{ready ? "WebLLM Ready" : "Loading WebLLM..."}</Text>
      <TextInput
...
// 超ざっくり: 事前生成した JSON index を使う
async function retrieveContext(query: string): Promise<string> {
  const [chunks, embeds] = await Promise.all([
    fetch("/client_side_llm/rag/corpus_chunks.json").then((r) => r.json()),
    fetch("/client_side_llm/rag/corpus_embeddings.json").then((r) => r.json()),
  ]);
  // ここで類似度計算して上位k件のtextを連結（実装は後で詰めればOK）
  return chunks.slice(0, 5).map((c: any) => c.text).join("\n");
}
```
