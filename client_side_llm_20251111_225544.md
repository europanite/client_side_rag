<!-- P2M_REPORT -->
<!-- GENERATED at 2025-11-11 22:55:44 -->
...
- Files: **36**
- Total size: **45807 bytes**
- Total LOC: 1396 | SLOC: 1170 | TODOs: 0
...
- `LICENSE` — 11340 bytes
- `frontend/app/screens/HomeScreen.tsx` — 9998 bytes
...
- `frontend/app/screens/HomeScreen.tsx` — 321 LOC
- `.gitignore` — 207 LOC
- `LICENSE` — 201 LOC
...
- Size: 20 bytes | LOC: 3 | SLOC: 3 | TODOs: 0 | Modified: 2025-11-11 22:31:24 | SHA1: e65a108cbb97
...
- Size: 756 bytes | LOC: 13 | SLOC: 8 | TODOs: 0 | Modified: 2025-11-11 22:31:24 | SHA1: 4cc074eb3727
...
- Size: 9998 bytes | LOC: 321 | SLOC: 266 | TODOs: 0 | Modified: 2025-11-11 22:49:57 | SHA1: 49802aff11d8

#### Brief
screens/HomeScreen.tsx
RAG playground with WebLLM (WebGPU) + retrieval-only fallback.
...
export default function HomeScreen() {
  ...
  // ---------- Init RAG index (shared by both modes) ----------
  useEffect(() => {
    (async () => {
      try {
        // 相対パスにすることで GitHub Pages のベースURL配下でも動く
        const [chunksRes, embedsRes] = await Promise.all([
          fetch("rag/corpus_chunks.json"),
          fetch("rag/corpus_embeddings.json"),
        ]);

        if (!chunksRes.ok || !embedsRes.ok) {
          throw new Error(
            `Failed to load RAG data: ${chunksRes.status}/${embedsRes.status}`
          );
        }

        const chunksJson = await chunksRes.json();
        const embedsJson = await embedsRes.json();

        setChunks(chunksJson as RagChunk[]);
        setEmbeddings(embedsJson as number[][]);
      } catch (err: any) {
        console.error("RAG index load error", err);
        setRagError("Failed to load RAG index (rag/*.json).");
      }
    })();
  }, []);

  // ---------- Init WebLLM engine or fallback ----------
  useEffect(() => {
    (async () => {
      // SSR / テスト環境などでは無理に初期化しない
      if (typeof window === "undefined") {
        setEngineMode("fallback");
        return;
      }

      // WebGPU チェック（公式 docs に従う）:contentReference[oaicite:1]{index=1}
      const hasWebGPU =
      ...
      if (!hasWebGPU) {
        console.warn("WebGPU not available. Falling back to retrieval-only mode.");
        setInitError(
          "WebGPU is not available on this device/browser. Using retrieval-only RAG."
        );
        setEngineMode("fallback");
        return;
      }
      ...
      try {
        ...
        setInitError(
          err?.message ||
            "Failed to initialize WebLLM. Falling back to retrieval-only RAG."
        );
        setEngineMode("fallback");
      }
    })();
  ...
  // ---------- RAG: simple vector or lexical retrieval ----------
  async function retrieveContext(query: string, topK = 5): Promise<RagChunk[]> {
    if (!chunks || !chunks.length) return [];

    // embeddings があればコサイン類似度、それ以外は超雑なTFっぽいスコア
  ...
  async function handleAsk() {
    ...
    if (!chunks) {
      setAnswer("RAG index not loaded yet.");
      return;
    }
    ...
    try {
      ...
      if (engineMode === "webllm" && engine) {
        ...
        // Fallback: LLMなし。RAGの「retrieve」結果をそのまま見せる。
        // → 「WebGPUない環境でも動く RAG 練習用ビュー」
        if (!ctxChunks.length) {
          setAnswer(
  ...
  function simpleTextToVector(text: string): number[] {
    // 超簡易：文字コード合計とかではなく、単語hashで固定長vector
    const dim = 64;
...
<a id="README.md"></a>
### 36. `README.md`
- Size: 906 bytes | LOC: 49 | SLOC: 31 | TODOs: 0 | Modified: 2025-11-11 22:29:50 | SHA1: 8fcbc63a2f8c
